...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, bn_enable=True):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, bn_enable=bn_enable)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, bn_enable=bn_enable)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2, bn_enable=bn_enable)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2, bn_enable=bn_enable)
...
def resnet18(pretrained=None, num_classes=None, bn_enable=True):
    return ResNet(BasicBlock, [2, 2, 2, 2],  num_classes=num_classes, bn_enable=bn_enable)
