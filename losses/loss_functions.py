...
def compute_normal_loss(params, model, criterion, inputs,labels, grads):
    # t = time.perf_counter() time.perf_count()是返回程序开始运行到调用这个语句所经历的时间。可以计算运行的效率
    outputs = model(inputs)
    # record_time(params, t, 'forward')  不必要，这是计时的
    loss = criterion(outputs, labels)                                                                 # 这里计算损失是为了进行反向传播
    if not params.dp:
        loss = loss.mean()                                                                            # 求平均值
    if grads:
        #t = time.perf_counter()
        grads = list(torch.autograd.grad(loss.mean(), [x for x in model.parameters() if x.requires_grad],retain_graph=True))
        #record_time(params, t, 'backward')
    return loss, grads


def compute_nc_evasion_loss(params, nc_model: Model, model: Model, inputs,labels, grads=None):
...
def compute_backdoor_loss(params, model, criterion, inputs_back, labels_back, grads=None):
    # t = time.perf_counter()
    ...
    if grads:
        grads = get_grads(params, model, loss)                                              # 梯度计算是一样的
